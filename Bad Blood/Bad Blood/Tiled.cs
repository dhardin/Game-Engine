...
namespace Squared.Tiled {
    ...
    public void Draw(SpriteBatch batch, Vector2 offset, Vector2 viewportPosition, Game.GameObject obj, float opacity)
        {
            Vector2 viewPos = viewportPosition;

            int minX = (int)Math.Floor(viewportPosition.X);
            int minY = (int)Math.Floor(viewportPosition.Y);
            int maxX = (int)Math.Ceiling((obj.Width + viewportPosition.X));
            int maxY = (int)Math.Ceiling((obj.Height + viewportPosition.Y));
            ...
            if (this.X + offset.X + this.Width > minX && this.X + offset.X < maxX)
                if (this.Y + offset.Y + this.Height > minY && this.Y + offset.Y < maxY)
                {
                    int x = (int)(this.X + offset.X - viewportPosition.X);
                    int y = (int)(this.Y + offset.Y - viewportPosition.Y);
                    batch.Draw(_Texture, new Rectangle(x, y, this.Width, this.Height), new Rectangle(0, 0, obj.Width, _Texture.Height),Color.White * opacity, obj.Rotation, obj.Orgin, SpriteEffects.None,0);
                }
        }
    }
